...
- `subset`: TrueType subsetting for glyf/loca/hmtx/vmtx/head/hhea/vhea/maxp/cmap (composite remap; long loca) + CFF1 charstrings/charset subsetting + CFF2 outline subsetting + passthrough metadata/axis tables (OS/2 with updated first/last char indices, gasp, cvt, fpgm, prep, VDMX, fvar, avar, STAT, MVAR, cvar, CPAL, meta) + VORG rebuild + kern format 0/2 rebuild + COLR v0 rebuild + GDEF class/attach/lig caret/mark set rebuild. Layout/color/variation tables that reference glyph IDs are preserved only for identity subsets.
...
| `subset` | Subsetting pipeline | `hb-subset*` | done (TrueType glyf/loca + vertical metrics + CFF1 charstrings/charset + CFF2 outlines + metadata/axis passthrough tables + VORG + kern format 0/2 + COLR v0 + GDEF class/attach/lig caret/mark sets; identity-only passthrough for layout/color/variation tables that reference glyph IDs) |
