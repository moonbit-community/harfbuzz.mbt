...
- `subset`: TrueType subsetting for glyf/loca/hmtx/head/hhea/maxp/cmap (composite remap; long loca) + CFF1 charstrings/charset subsetting (CFF2 cmap-only).
...
| `subset` | Subsetting pipeline | `hb-subset*` | done (TrueType glyf/loca + CFF1 charstrings/charset; CFF2 cmap-only) |
