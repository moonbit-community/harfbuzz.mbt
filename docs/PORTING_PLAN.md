...
| `subset` | Subsetting pipeline | `hb-subset*` | done (TrueType glyf/loca + vertical metrics + CFF1 charstrings/charset + CFF2 outlines + metadata/axis passthrough tables + VORG + kern format 0/2/3 + COLR v0 + GDEF class/attach/lig caret/mark sets + GSUB lookup types 1/2/3/4/5/6; identity-only passthrough for layout/color/variation tables that reference glyph IDs) |
