...
GSUB lookup types 1/2/3/4/5/6 (single/multiple/alternate/ligature/contextual/
chaining substitution) are rebuilt for remapped subsets; other GSUB/GPOS lookups
are dropped.
...
- Layout/color/variation tables that reference glyph IDs are not subset yet
  ...
  lookup types 1/2/3/4/5/6, and `kern` format 0/2/3); they are only preserved when the
